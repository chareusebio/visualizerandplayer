...
function Songs({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  function handleSubmit() {
    let searchedSongs = (document.getElementById('searchbar') as HTMLInputElement);
    if (searchedSongs.value !== '') {
      setSearched(true);
      songs.map(song => {
        if (song.get('songTitle').toLowerCase() === searchedSongs.value.toString().toLowerCase() || song.get('songTitle') === 'Stop') {
          array.push(song);
        }
      });
      setUserSearch(array);
  ...
  function SearchedSongs(): JSX.Element {
    return (
      <div>
        {userSearch.map((song: { get: (arg0: string) => {} | null | undefined | any }) => (
          <div
            key={song.get('id')}
            className="f6 pointer underline flex items-center no-underline i dim"
            onClick={() =>
  ...
  return (
    <Section title="Playlist">
      <div id='search'>
        <input type="text" placeholder="Search a song" id='searchbar' />
        <Search32 id='searchbutton' onClick={() => handleSubmit()} />
      </div>
      {searched ? (
        <SearchedSongs />
      ) : (
